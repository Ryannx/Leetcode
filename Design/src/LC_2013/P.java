...
public class P {
    ...
    public int[] detect(String[] cmd, int[][][] input) {
        ...
        for (int i = 0; i < cmd.length; i++) {
            comand.add(cmd[i]);
            if (input[i].length == 0) {
                position.add(Arrays.asList(-1, -1));
                continue;
            }
            if (input[i][0][0] == 1 && input[i][0][1] == 9) {
                System.out.println(cmd[i]);
            }
            position.add(Arrays.asList(input[i][0][0], input[i][0][1]));
            if (cmd[i].equals("count")) {
                res++;
                if (res == 12) {
                    idx = i;
                    break;
